...
const COLLECTION_PATH = ["artifacts", "ultra-study-point", "public", "data", "PYQs"];
...
export default function PyqsListPage() {
  ...
  async function cleanupExpiredTrash() {
    ...
    const snap = await getDocs(query(collection(db, ...COLLECTION_PATH), where("isDeleted", "==", true), limit(200)));
    const now = Date.now();
  ...
  async function loadPage(next = false) {
    ...
    const q = query(collection(db, ...COLLECTION_PATH), orderBy("updatedAt", "desc"), ...(next && lastDoc ? [startAfter(lastDoc)] : []), limit(50));
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}>PYQs</h1>
          <p style={styles.subtitle}>Manage previous year question sets and question bank</p>
        </div>
        <button onClick={() => router.push("/admin/pyqs/create")} style={styles.createBtn}>+ Create PYQ</button>
      </div>
      ...
      <div style={styles.filters}>
        {["all", "draft", "review", "published", "hidden", ...(isSuperAdmin ? ["recently_deleted"] : [])].map((s) => (
            ...
            <tbody>
              {filtered.map((item) => (
                <tr key={item.id} style={styles.row}>
                  {isSuperAdmin && <td style={styles.td}><input type="checkbox" checked={Boolean(selectedIds[item.id])} onChange={(e) => setSelectedIds((prev) => ({ ...prev, [item.id]: e.target.checked }))} /></td>}
                  <td style={styles.slug}>{item.title || item.id}</td>
                  <td style={styles.td}>{item.exam || "-"}</td>
                  ...
                  <td style={styles.td}>
                    {filter !== "recently_deleted" && (
                      <>
                        <button style={styles.iconBtn} onClick={() => router.push(`/admin/pyqs/${item.id}`)}>Edit</button>
...
const styles = {
  ...
  status: (status) => ({
    ...
    background: status === "published" ? "#dcfce7" : status === "review" ? "#fef9c3" : status === "hidden" ? "#fee2e2" : "#e5e7eb",
    color: status === "published" ? "#166534" : status === "review" ? "#92400e" : status === "hidden" ? "#b91c1c" : "#374151",
  }),
