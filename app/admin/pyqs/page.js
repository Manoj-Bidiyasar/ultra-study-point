...
export default function PyqsListPage() {
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        <div>
          ...
          <p style={styles.subtitle}>
            Manage previous year question sets and question bank
          </p>
        </div>

        <button
          onClick={() => router.push("/admin/pyqs/create")}
      ...
      {!loading && filtered.length === 0 && <p>No PYQs found.</p>}
            ...
            <tbody>
              {filtered.map((item) => (
                <tr
                  key={item.id}
                  style={styles.row}
                  onClick={() => router.push(`/admin/pyqs/${item.id}`)}
                >
                  <td style={styles.slug}>{item.title || item.id}</td>
                  <td style={styles.td}>{item.exam || "â€”"}</td>
                  <td style={styles.td}>{item.year || "â€”"}</td>
                  <td style={styles.td}>
