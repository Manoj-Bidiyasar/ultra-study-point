...
const COLLECTION_PATH = ["artifacts", "ultra-study-point", "public", "data", "master_notes"];
    ...
    <div style={styles.page}>
      ...
      <div style={styles.header}>
        <div>
          ...
          <p style={styles.subtitle}>Manage master notes and study material</p>
        </div>
        <button onClick={() => router.push("/admin/notes/create")} style={styles.createBtn}>+ Create Note</button>
      </div>
      ...
      <div style={styles.filters}>
        {["all", "draft", "review", "published", ...(isSuperAdmin ? ["recently_deleted"] : [])].map((s) => (
            ...
            <tbody>
              {filtered.map((item) => (
                <tr key={item.id} style={styles.row}>
                  {isSuperAdmin && (
                    <td style={styles.td}><input type="checkbox" checked={Boolean(selectedIds[item.id])} onChange={(e) => setSelectedIds((prev) => ({ ...prev, [item.id]: e.target.checked }))} /></td>
                  )}
                  ...
                  <td style={styles.td}>
                    {filter !== "recently_deleted" && (
                      <>
                        <button style={styles.iconBtn} onClick={() => router.push(`/admin/notes/${item.id}`)}>Edit</button>
...
const styles = {
  ...
  status: (status) => ({
    ...
    background: status === "published" ? "#dcfce7" : status === "review" ? "#fef9c3" : "#e5e7eb",
    color: status === "published" ? "#166534" : status === "review" ? "#92400e" : "#374151",
  }),
