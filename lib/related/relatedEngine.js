...
import { getLatestPyqs } from "./pyqSelectors";
...
const resolveAutoRelatedContent = unstable_cache(
  async ({ pageType, pageCaDate, subject, tags, device }) => {
    ...
    const latestQuizzes = await getLatestQuizzes({ limitCount: 3 });
    const latestPyqs = await getLatestPyqs({ limitCount: 2 });
...
export async function resolveRelatedContent({
  ...
  return {
    ...
    latestQuizzes: auto.latestQuizzes || [],
    pyqs: auto.pyqs || [],
  };
}
