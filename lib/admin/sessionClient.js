...
export async function startAdminSession({ uid, userData, getIdToken }) {
  ...
  const sessionRef = doc(db, ...USER_PATH, uid, "sessions", sessionId);
  const payload = {
    uid,
    deviceId,
    userAgent:
  ...
  try {
    ...
    // Auth token propagation race can cause temporary permission-denied right after login.
    ...
    try {
      ...
      if (String(err2?.code || "") !== "permission-denied") throw err2;
      // Final fallback: create session with Admin SDK through API.
      let token = "";
      ...
      if (!res.ok || !data?.ok || !data?.sessionId) {
        const e = new Error(data?.error || "admin-session-fallback-failed");
        e.code = data?.error || "permission-denied";
        throw e;
      }

      if (typeof window !== "undefined") {
        window.localStorage.setItem("admin_session_id", data.sessionId);
      }
      return { sessionId: data.sessionId, deviceId: data.deviceId || deviceId };
    }
  }
